...
class EntityInteractLog : LogType {
    private val logger: Logger = Logger.getLogger(EntityInteractLog::class.java.name)
    private val objectMapper = ObjectMapper()

    override val actionType: String = "ENTITY"

    override fun log(playerUUID: UUID, details: String) {
        try {
            DatabaseManager.logPlayerAction(playerUUID, actionType, details)
        } catch (e: Exception) {
            logger.severe("Failed to log entity interaction for $playerUUID: ${e.message}")
        }
    }
    ...
    override fun toJson(eventData: Map<String, Any>): String {
        return try {
            ...
            logger.warning("Failed to convert entity interaction data to JSON: ${e.message}")
            "{}"
        }
    }
    ...
    override fun toSentence(jsonDetails: String): String {
        return try {
            ...
            "$issuer interacted with $entityType at $location in $world$cancelledText"
        } catch (e: Exception) {
            logger.warning("Failed to parse JSON details: ${e.message}")
            "Invalid entity interaction log"
        }
    }
}