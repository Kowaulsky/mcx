...
class ItemDropLog : LogType {
    private val logger: Logger = Logger.getLogger(ItemDropLog::class.java.name)
    ...
    override fun log(playerUUID: UUID, details: String) {
        try {
            ...
            logger.severe("Failed to log item drop for $playerUUID: ${e.message}")
        }
    }
    ...
    override fun toJson(eventData: Map<String, Any>): String {
        return try {
            ...
            logger.warning("Failed to convert item drop data to JSON: ${e.message}")
            "{}"
        }
    }
    ...
    override fun toSentence(jsonDetails: String): String {
        return try {
            ...
            "$issuer dropped $amount $itemType at $location in $world$cancelledText"
        } catch (e: Exception) {
            logger.warning("Failed to parse JSON details: ${e.message}")
            "Invalid item drop log"
        }
    }
}